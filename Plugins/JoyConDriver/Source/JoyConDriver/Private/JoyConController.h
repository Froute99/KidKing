...
struct FRumble {
	...
	void CalculateRumbleData() {
		...
		if (Amplitude == 0.0f) {
			...
			const uint16 HighFrequencyLocal = static_cast<uint16>((FMath::RoundToInt32(32.0f * FMath::LogX(2, HighFrequency * 0.1f)) - 0x60) * 4);
			const uint8 LowFrequencyLocal = static_cast<uint8>(FMath::RoundToInt32(32.0f * FMath::LogX(2, LowFrequency * 0.1f)) - 0x40);
			...
			uint16 LowFrequencyAmplitude = static_cast<uint16>(FMath::RoundToInt64(HighFrequencyAmplitude) * .5);
